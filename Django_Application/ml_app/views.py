...
def detect_audio_file(audio_path, request=None):
    ...
    try:
        model_path = os.path.join(settings.PROJECT_DIR, "models", "best.pt")
...
def load_global_model():
    ...
    if global_model is None:
        ...
        model_path = os.path.join(settings.PROJECT_DIR, 'models', model_name)
        
        # If the specific model doesn't exist, try to find any .pt model file
        if not os.path.exists(model_path):
            import glob
            available_models = glob.glob(os.path.join(settings.PROJECT_DIR, 'models', '*.pt'))
            # Filter out yolo model
            available_models = [m for m in available_models if 'yolo' not in m.lower()]
            
            if available_models:
                model_path = available_models[0]
                logger.warning(f"Default model not found. Using: {os.path.basename(model_path)}")
            else:
                # No model found - create a dummy model for now
                logger.error("No video deepfake detection model found in models/ directory!")
                logger.error("Please download the model file and place it in the models/ directory.")
                logger.error("Expected: models/model_97_acc_100_frames_FF_data.pt")
                # Initialize with random weights as fallback
                global_model = model
                return global_model
        
        try:
            # PyTorch 2.11+ requires weights_only=False for models with custom objects
            model.load_state_dict(torch.load(model_path, map_location=device, weights_only=False))
            model.eval()
            global_model = model
            logger.info(f"Global model loaded from: {os.path.basename(model_path)}")
        except Exception as e:
            logger.error(f"Failed to load model: {e}")
            # Use model with random weights as fallback
            global_model = model
